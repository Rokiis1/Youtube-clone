...
import { convertViewstoString, timeSince } from "../../utils";
import { YOUTUBE_API_URL } from "../../config/constants";

const API_KEY = import.meta.env.VITE_YOUTUBE_DATA_API_KEY;

export const getVideoDetails = createAsyncThunk(
  "yotubeState/videoDetails",
  async (id: string) => {
    const {
      data: { items },
    } = await axios.get(
...
const parseData = async (item: {
  ...
  return {
    ...
    videoLikes: convertViewstoString(item.statistics.likeCount),
    videoAge: timeSince(new Date(item.snippet.publishedAt)),
    channelInfo: {
      id: item.snippet.channelId,
      image: channelImage,
      name: item.snippet.channelTitle,
      subscribers: convertViewstoString(subscriberCount, true),
    },
  };
};
